...
package cn.zhuatech.siem.config;
import cn.zhuatech.siem.domain.DomainCatalog;
import cn.zhuatech.siem.model.*;
import cn.zhuatech.siem.repository.*;
...
@Configuration public class DataInitializer {
 @Bean CommandLineRunner seed(BusinessRecordRepository records,SystemSettingRepository settings,DomainCatalog catalog) {
  return args -> { if(records.count()>0)return;
            ...
            settings.save(new SystemSetting("recordNoRule", "SIEM-{YYYY}-{SEQ}"));
   ...
   int sequence=1; for(var module:catalog.modules()) {
    String no="SIEM-DEMO-"+String.format("%03d",sequence);
