...
| POST | `/domain/decision` | 执行安全信息与事件管理系统专属领域规则 |
...
| --- | --- | --- |
| `eventNo` | String | 事件编号 |
| `severity` | int | 严重等级(1-5) |
| `assetCriticality` | int | 资产关键度(1-5) |
| `confidence` | double | 检测置信度(%) |
| `affectedAccounts` | int | 受影响账号数 |
| `privilegedAccount` | boolean | 涉及特权账号 |
